...
const firstResponderButton = '#first_responder_button'; 
const firstResponderOnFocusButton = '#first_responder_on_focus_button'; 
const keyboardResponderList = '.keyboard-responder-list';
const activateOnFocus = '.activate-on-focus';
const firstResponderOnFocus = '.first-responder-on-focus';
...
test('test mixin functionality', function(assert) {
  visit('/').then(() => {
    ...
    return click(firstResponderButton);
  }).then(() => {
    assert.equal(Ember.$(keyboardResponderList).find('li').first().find(keyboardPriority).text().trim(), '9999999999999', 'first-responder has firstResponder priority');

    return click(firstResponderOnFocusButton);
  }).then(() => {
    assert.equal(Ember.$(keyboardResponderList).find('li').first().find(responderName).text().trim(), 'KeyboardFirstResponderMixin', 'first-responder still at the top of the list');

    return triggerEvent(firstResponderOnFocus, 'focus');
  }).then(() => {
    assert.equal(Ember.$(keyboardResponderList).find('li').first().find(responderName).text().trim(), 'KeyboardFirstResponderOnFocusMixin', 'first-responder-on-focus becomes first responder');
    ...
    return triggerEvent(firstResponderOnFocus, 'blur');
  }).then(() => {
